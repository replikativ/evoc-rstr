...
;; n-1 edges (weights = euclidean mutual-reachability, i.e. sqrt of rdist); returns
;; the edge count.
(deftm prim-mst!
...
(defn mst
  ...
  (if (clojure.core/< (long n) boruvka-threshold)
    ...
    ((requiring-resolve 'evoc.boruvka/boruvka-mst) emb n dim min-samples)))

;; Thin orchestrator: kNN -> core distances -> dense Prim MST.
;; Returns {:from int[] :to int[] :w double[]} (n-1 edges, weights = euclidean
;; mutual-reachability, i.e. sqrt of rdist).
